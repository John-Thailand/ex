...
class _HealthAppState extends State<MyHealthPage> {
    ...
    if (requested) {
      try {
        ...
        List<HealthDataPoint> healthData = await health.getHealthDataFromTypes(
            todayFrom /*yesterday*/, now, types);
        // save all the new data points (only the first 100)
        int max = 300;
        _healthDataList.addAll((healthData.length < max)
            ? healthData
            : healthData.sublist(0, max));
      } catch (error) {
  ...
  Future addData() async {
    ...
    final permissions = [
      ...
      HealthDataAccess.READ_WRITE,
      HealthDataAccess.READ_WRITE,
      HealthDataAccess.READ_WRITE, //miura++
      HealthDataAccess.READ_WRITE, //miura++
      HealthDataAccess.READ_WRITE, //miura++
      // HealthDataAccess.READ_WRITE,
    ];
  ...
  Future deleteData() async {
    ...
    success &= await health.delete(
        HealthDataType.SLEEP_ASLEEP, earlier, now); //miura++
    success &=
        await health.delete(HealthDataType.SLEEP_AWAKE, earlier, now); //miura++
    success &= await health.delete(
        HealthDataType.SLEEP_IN_BED, earlier, now); //miura++
    success &= await health.delete(
        HealthDataType.BLOOD_PRESSURE_SYSTOLIC,
        earlier,
